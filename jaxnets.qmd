  ...
  for folder_name in folder_list:
    ...
    for file_name in file_list:
      if file_name.endswith('.npz'):
        file = np.load(os.path.join(full_folder_path, file_name), allow_pickle=True)
        file_data = {key: file[key].item() if file[key].size == 1 else np.mean(file[key]) for key in file.files}
