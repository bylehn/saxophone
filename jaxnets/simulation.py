...
def generate_auxetic(run, number_of_nodes_per_side, perturbation, opt_steps):
    ...
    success_frac=0.05
    k_fit = 2.0/(dw**2) 
    system = utils.System(number_of_nodes_per_side, 26+run, 2.0, 0.2, 0.1)
    system.initialize()
    system.acoustic_parameters(w_c, dw, nr_trials, ageing_rate, success_frac)
    system.auxetic_parameters(perturbation, delta_perturbation, steps, write_every)
    displacement = system.displacement
...
def generate_nauxetic(run, number_of_nodes_per_side, perturbation, opt_steps):

    """
    a version of generate_auxetic with the opposite goal
    """
    steps = 50
    write_every = 1
    delta_perturbation = 0.1
